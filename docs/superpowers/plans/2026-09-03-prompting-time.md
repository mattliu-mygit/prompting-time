...
Expected: all required methods appear. Do not commit the full generated schema. Build a small fixture containing initialize/initialized, thread start, turn start, agent delta, child-agent notification when available, approval request/resolution, and turn completion. Replace real paths, IDs, messages, and account data with invented values before staging.

- [ ] **Step 2: Write adapter parsing and request-correlation tests**

Feed the fixture through an in-memory transport. Assert JSON-RPC response IDs resolve the correct pending request, notifications normalize to the correct run/agent event, unknown notifications are retained as `ProviderEvent::Unrecognized` without failing the stream, and request errors preserve code/message without provider payload leakage.
...
Pass the recorded workspace directory and explicit permission policy; never use danger-full-access or bypass approvals by default. Persist `thread.id`, `thread.sessionId`, active turn ID, and native item IDs.
