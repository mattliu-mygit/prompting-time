...
Prompting Time is a local macOS desktop application for managing coding-agent conversations across multiple installed CLI harnesses. The current release candidate integrates Codex App Server. It has no Claude adapter: Claude Code is discovered and shown with an actionable diagnostic, but remains unavailable for execution because its unauthenticated protocol probes were inconclusive. The intended multi-provider release gives the user one place to create, monitor, steer, interrupt, resume, and switch providers across concurrent conversations without giving up either harness's native tools, authentication, approvals, or session persistence.
...
- Codex integration through its installed local CLI, plus a visible but protocol-gated Claude Code provider boundary.
...
- Provider switching between turns with explicit context handoff. The current release candidate implements and hermetically tests the provider-neutral machinery, but live Codex-to-Claude switching remains unavailable and unverified.
...
The provider boundary defines one capability-aware adapter contract covering discovery and health, session start and resume, message submission, active-turn steering when supported, interruption, approval or user-input responses, normalized event streaming, and capability reporting. Codex implements that contract. Claude does not yet have an adapter and is represented only by discovery diagnostics and protocol-gate probes.
...
The Codex adapter runs through a shared behavioral contract suite. Fake adapters and child processes exercise provider-neutral switching and handoff, streaming, malformed events, cancellation, process crashes, delayed approvals, backpressure, and resume boundaries without consuming provider usage. Claude tests are protocol probes, not an adapter contract suite; their unauthenticated live result is inconclusive.

Integration tests use temporary SQLite databases and temporary Git repositories to verify migrations, transactional recovery, concurrent dispatch claims, worktree creation, dirty-state protection, divergence detection, and cleanup. Recorded sanitized fixtures validate supported Codex shapes and Claude probe parsing, not a working Claude adapter.
...
Focused opt-in smoke tests are intended to validate discovery, authentication status, one projectless conversation, one isolated-worktree conversation, streaming, interruption, approval, resumption, and provider switching against installed provider CLIs. They are reported separately from hermetic automated tests. The current release evidence does not include a completed account-backed Codex flow, an authenticated Claude flow, live Codex-to-Claude switching, or a native visual walkthrough.
