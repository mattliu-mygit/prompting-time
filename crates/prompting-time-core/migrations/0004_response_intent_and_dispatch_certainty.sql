...
ADD COLUMN dispatch_certainty TEXT CHECK (
    dispatch_certainty IS NULL
    OR dispatch_certainty IN ('not_dispatched', 'may_have_dispatched')
);
...
INSERT INTO approvals_v4 (
    ...
    status, resolution_json, response_intent_json, response_intent_status,
    created_at, updated_at
)
SELECT
    id, run_id, agent_id, provider, provider_request_id, operation, scope,
    status, resolution_json, NULL, NULL, created_at, updated_at
FROM approvals;
